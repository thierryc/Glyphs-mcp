...
Agent plugins are a separate, optional setup. Version 2.0.0 includes one shared
...
- [Latest installer DMG](https://github.com/thierryc/Glyphs-mcp/releases/latest/download/Glyphs-MCP-latest.dmg)
...
- Uses one runtime path plan everywhere: embedded Glyphs Python installs into
  and prioritizes that target's `Scripts/site-packages`; external Python
  installs into and prioritizes its user site, with the Glyphs path as fallback.
- Stops when the selected package copy has an incompatible Python ABI or Mac
  architecture, while reporting stale lower-priority copies as warnings.

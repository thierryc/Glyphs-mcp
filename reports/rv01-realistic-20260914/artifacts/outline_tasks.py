...
# Qualification exposed native grid rounding. Preserve the transient layer flag,
# using the same native API already qualified by the installed bridge; no imports
# from the bridge and no document settings or history changes.
def precise_action(action,glyph):
 def run(font,master_ids):
  targets=layers(font,glyph,master_ids);flags=[]
  try:
   for l in targets:
    flag=l.temporarilyDisableRounding() if callable(l.temporarilyDisableRounding) else l.temporarilyDisableRounding
    flags.append((l,bool(flag)));l.setTemporarilyDisableRounding_(True)
   return action(font,master_ids)
  finally:
   for l,flag in reversed(flags):l.setTemporarilyDisableRounding_(flag)
 return run
adjust_handle=precise_action(adjust_handle,'o')
add_stem_midpoint=precise_action(add_stem_midpoint,'H')
remove_stem_midpoint=precise_action(remove_stem_midpoint,'H')
split_o_midpoint=precise_action(split_o_midpoint,'o')

add_extrema=precise_action(add_extrema,'at')

...
description: Create and review workspace-first native Glyphs scripts and companion plugins.
metadata:
...
Create scripts and plugins in the workspace. Inspect the pinned GlyphsSDK
and its public APIs before using native selectors. Preserve the SDK loader
and attribution. Keep pure geometry separate from native UI, redraw from
native events, and avoid work in drawing callbacks. Installation and execution
require authorization from the user's task. Never promise automatic full-font
recovery for arbitrary Python. Run focused tests and the native disposable gate.

...
public struct InstallerRuntimeIdentity: Codable, Equatable, Sendable {
...
public struct ResolvedInstallerTargetPayload: Equatable, Sendable {
	...
	public let runtimeIdentity: InstallerRuntimeIdentity?
}
...
public enum InstallerPayloadManifestResolver {
	private struct Manifest: Decodable {
		...
		struct Target: Decodable {
			...
			let runtimeIdentity: InstallerRuntimeIdentity?
		}
	...
	public static func resolve(_ payloadDirectory: URL) throws -> ResolvedInstallerPayloadManifest {
		...
		if manifest.schemaVersion >= 3 {
            ...
            for directory in ["Lean", "Installer"] {
                guard let expected = fields?[directory.lowercased() + "Identity"] as? String,
                      try treeIdentity(payloadDirectory.appendingPathComponent(directory)) == expected else {
		...
		for glyphsMajor in manifest.schemaVersion == 4 ? [4] : [3, 4] {
			...
			if glyphsMajor == 4 && manifest.schemaVersion == 2 {
				guard let identity = target.runtimeIdentity,
					  identity.version == target.pluginVersion,
					  identity.codeHash.range(
						  of: "^[0-9a-f]{64}$",
						  options: .regularExpression
					  ) != nil,
					  identity.runtimeId == "\(identity.version)+\(identity.codeHash.prefix(12))",
					  identity.hashScope == "Resources/**/*.py and Contents/Info.plist" else {
					throw UpdateStagingError("payload_manifest", "Glyphs 4 runtime identity is missing or malformed.")
				}
				let observedCodeHash = try runtimeCodeHash(bundleURL: bundleURL)
				guard observedCodeHash == identity.codeHash else {
					throw UpdateStagingError(
						"payload_manifest",
						"Glyphs 4 runtime identity is stale or does not match its files " +
							"(expected \(identity.codeHash), observed \(observedCodeHash))."
					)
				}
			}
    ...
    public static func treeIdentity(_ root: URL) throws -> String {
        guard let enumerator = FileManager.default.enumerator(atPath: root.path) else {
            throw UpdateStagingError("payload_manifest", "Missing component directory.")
        }
        var files: [String] = []
        while let relative = enumerator.nextObject() as? String {
            ...
            guard values.isSymbolicLink != true else {
                throw UpdateStagingError("payload_manifest", "Packaged components must not contain symbolic links.")
            }
            if values.isRegularFile == true { files.append(relative) }
        }
        var hash = SHA256()
        for relative in files.sorted() {
            ...
            let file = try FileHandle(forReadingFrom: root.appendingPathComponent(relative))
	...
	static func runtimeCodeHash(bundleURL: URL) throws -> String {
		...
		// Enumerating at a path returns relative names directly.  Deriving them
		// from absolute URLs is incorrect on macOS because FileManager may return
		// /private/tmp children for a /tmp root.
		guard let enumerator = FileManager.default.enumerator(atPath: resources.path) else {
			throw UpdateStagingError("payload_manifest", "Glyphs 4 runtime Resources are missing.")
		}
		var entries: [(String, URL)] = []
		while let relative = enumerator.nextObject() as? String {
			let url = resources.appendingPathComponent(relative)
			let components = relative.split(separator: "/").map(String.init)
			if components.contains("vendor") || components.contains("__pycache__") {
			...
			guard values.isSymbolicLink != true, values.isRegularFile == true else {
				throw UpdateStagingError("payload_manifest", "Glyphs 4 runtime hash scope contains an unsafe file.")
			}
			entries.append((relative, url))
		}
		entries.sort { $0.0 < $1.0 }
		entries.append(("../Info.plist", infoPlist))
...
public enum InstallerPayloadArchiveValidator {
	public static func validateListing(_ listing: String) throws {
		...
		guard required.allSatisfy(entries.contains),
              entries.contains("Payload/Plugins/Glyphs4/Glyphs MCP.glyphsPlugin/Contents/Info.plist") ||
              entries.contains("Payload/Lean/Glyphs MCP Bridge.glyphsPlugin/Contents/Info.plist") else {
	...
	public static func live(
		...
		let verifyArchive: @Sendable (URL, String, Int) throws -> UpdateVerifiedPlugin = { extractedRoot, expectedVersion, glyphsMajor in
            ...
            guard payload.schemaVersion < 3 else {
                throw UpdateStagingError("installer_required", "Open the Glyphs MCP installer to update the runtime, bridge and selected companions together.")
            }

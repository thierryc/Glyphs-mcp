...
sys.path[:0] = [str(ROOT / 'src/sidecar'), str(ROOT / 'src/protocol'), str(ROOT / 'scripts')]
...
async def run(cycle, phase):
    ...
    async with Client(CONFIG.get('endpoint', 'http://127.0.0.1:9680/mcp/')) as client:
        ...
        assert status.get('controlProtocol') == 1, status
        evidence['desktopProtocol'] = status['controlProtocol']
        async def poll(job_id, terminals):
            samples, errors = [], []
            started = time.monotonic(); ended = None; next_status = started; activity = []
            while True:
                ...
                now = time.monotonic()
                if now >= next_status:
                    observed = await tool('get_status')
                    assert observed.get('controlProtocol') == 1, observed
                    activity.append({'at': round(now-started,3), 'worker': observed['worker'], 'activity': observed.get('activity')})
                    next_status = now + 2
            ...
            timing = dict(_measurement(samples), errors=errors, postOperationSeconds=2,
                          durationSeconds=round(time.monotonic()-started, 3), activity=activity)
        ...
        async def prepare(kind):
            ...
            job = await tool('get_job', job_id=resumed) if resumed else await tool('start_job', document_id=doc['id'], kind=kind, **kw)
            assert job['status'] in {'preparing','ready'}, job
            evidence.setdefault('preparedJobs', {})[phase] = job['id']
            if resumed: evidence.setdefault('resumedJobs', {})[phase] = resumed
            record()
        ...
        if phase == 'baseline':
        ...
        elif phase in {'undo','redo','native-revert','control-retained'}:
            ...
            if phase == 'control-retained':
                retained = await tool('get_job', job_id=evidence['jobId'])
                assert retained['status'] == 'applied', retained
                evidence['phases'][phase] = {'exact':True,'jobId':retained['id'],'status':retained['status']}
            else:

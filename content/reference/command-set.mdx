...
Note: the server can be configured with a **tool profile** (in the Glyphs plug-in status window) that hides tools you don't need for a given workflow. When a profile is active, `tools/list` will return a subset of the full tool surface.

...
def install(build, home, python, application, companions=(), *, mcp=True, remove_components=(), start=False):
    ...
    if (requested | removed) - set(COMPONENTS) or requested & removed:
    ...
    with (root/'.install.lock').open('a') as lock, ExitStack() as controls:
        ...
        sources.extend((build/bundles[name]['bundle'], plugins/bundles[name]['bundle'], bundles[name]['identity'])
                       for name in COMPONENTS if name in selected)
        return _replace(build, home, root, plugins, application, manifest, selected, removed, sources, python, runtime, start, controller)


def _replace(build, home, root, plugins, application, manifest, selected, removed, sources, python, runtime, start, controller):
    ...
    with tempfile.TemporaryDirectory(prefix='.prepare-', dir=root) as tmp:
        ...
        result = {'schemaVersion':2,'version':manifest['projectVersion'],'installerBuild':manifest['installerBuild'],'bridgeVersion':manifest['version'],
            ...
            'installed':[{'path':str(target),'identity':identity} for _,target,identity in sources],
        ...
        result['componentDetails'] = {name: {**manifest.get('components', {}).get(name, {}),
            'version': manifest.get('projectVersion', '2.0.0'), 'identity': bundles[name]['identity']} for name in selected}
        result['runtime'] = {**runtime, 'architecture':platform.machine(), 'installedPath':str(root/'runtime')} if runtime and selected & {'mcp','reference-inspector'} else None
        result['bridge'] = {**manifest['bridge'], 'version':manifest['version']} if 'mcp' in selected else None
        result['sidecar'] = {**manifest['sidecar'], 'requires':['runtime']} if 'mcp' in selected else None
        result['companions'] = [item for item in manifest['companions'] if item['id'] in selected]
...
def main():
    ...
    result=install(args.build,args.home,args.python,args.glyphs_app,args.companion,
                   mcp=not args.no_mcp,remove_components=args.remove,start=args.start)

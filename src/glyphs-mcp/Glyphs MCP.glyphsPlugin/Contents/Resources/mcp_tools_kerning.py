...
async def generate_kerning_tab(
    ...
    """Open a kerning review proof tab in Glyphs.
    ...
    Safety:
      - Does not change your font (no mutation, no auto-save).
      - Creates a new Edit tab in Glyphs with proof text.

    Units:
      - All kerning values and limits are in Glyphs font units.
    ...
    Returns:
        JSON payload with the generated text and metadata. Notable keys:
          - ok (bool)
          - fontIndex, masterId
          - dataset (id, usedTopN)
          - counts (pair coverage stats)
          - openedTab (bool)
          - text (proof tab content)
          - warnings (list[str])
    """
    try:
        if font_index >= len(Glyphs.fonts) or font_index < 0:
...
async def review_kerning_bumper(
    ...
    """Measure kerning collisions and propose conservative “bumper” exceptions.

    This tool performs geometry-based measurements (outline intersections) for a
    prioritized set of glyph pairs, then reports:
      - the minimum measured outline gap per pair, and
      - a recommended glyph–glyph kerning exception that would *loosen* kerning
        just enough to satisfy the configured `min_gap`.

    Safety:
      - Does not change your font (no mutation, no auto-save).
      - Optionally opens a proof tab when `open_tab=true`.

    Units:
      - `min_gap`, kerning values, and `dense_step` are in font units.
      - `scan_heights` are normalized positions (0..1) within the pair’s vertical overlap band.

    Performance:
      - Increase `pair_limit` and/or `dense_only` scans with care; this can get expensive on large fonts.

    Notes (typography):
      - Recommendations are a collision guard, not an optical kerning engine.
...
async def apply_kerning_bumper(
    ...
    Units:
      - `min_gap`, `extra_gap`, and `dense_step` are in font units.
      - `scan_heights` are normalized (0..1) within the overlap band.

    Behavior:
      - Only *loosens* kerning (never tightens).
      - Writes explicit glyph–glyph kerning exceptions in the given master.
      - Use `pairs` to target specific pairs; otherwise uses the prioritized dataset + existing kerning reps.

    Returns:
      JSON payload with per-pair actions and a summary. Notable keys:
        - ok (bool), dryRun (bool)
        - summary (counts + params)
        - results (per-pair applied/skipped/error)
    """
    try:

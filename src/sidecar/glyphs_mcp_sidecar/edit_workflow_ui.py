...
def register_edit_workflow_tools(mcp, service):
    ...
    def start_edit_workflow(
        ...
        """Preferred conversational route for an authorized font edit. Reuse a unique idempotency_key on retries. Same typed scope as start_job; mutation kinds only. apply mode automatically prepares and applies complete results; preview mode waits. Retain workflow_id from the JSON text control reference, even if structured data is unavailable. When poll is true call get_edit_workflow until a choice or outcome; Preparing is not Applied. Never saves implicitly: dirty/new fonts return choices. Present the MCP App where supported and always retain the text choices. 'Save and continue' saves current work once, NOT the resulting edit. Ask for saving only if not already authorized. No user should type tool IDs, tokens or JSON. Existing Dimensions overwrite consent remains required. On warnings/skips review the full get_job report before selecting Apply; do not claim partial work as complete."""

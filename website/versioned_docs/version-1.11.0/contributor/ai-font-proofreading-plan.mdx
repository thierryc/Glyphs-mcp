...
The shipped one-document MCP activity record and the ranked source-comparison
and hinting sequence are tracked in the public [Glyphs MCP roadmap](https://github.com/thierryc/Glyphs-mcp/blob/v1.11.0/ROADMAP.md).
That 1.9 ledger is deliberately memory-only and is not the persistent semantic
review session proposed below.

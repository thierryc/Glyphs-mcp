...
RESUME=globals().get('H3_RESUME',False)
if RESUME:
    facts=json.loads((OUT/globals().get('H3_RESUME_FILE','native-first-attempt.json')).read_text())
    facts.setdefault('harnessErrors',[]).append(facts.pop('error'))
    facts.pop('passed',None);facts.pop('finishedAt',None)
    facts['resumedAt']=datetime.now(timezone.utc).isoformat()
fonts={}
...
def run():
    try:
        ...
        if not RESUME:
            ...
            path_a=main.call(lambda:open_copy('A'));doc,_,_=discover(path_a)
            main.call(lambda:edit_selection('A','empty'))
        def repeated():
            f=fonts['A'];t=f.currentTab;t.layers=[f.glyphs[n].layers[f.masters[1].id] for n in ('control','mixed','control')];t.selectedTextRange=NSMakeRange(0,3)
        main.call(repeated);inspect('A',doc,'repeated occurrences')
        def empty_tab():
            f=fonts['A'];f.currentTab.layers=[]
        ...
        def dirty():
            f=fonts['A'];f.glyphs['control'].layers[f.masters[0].id].width+=.25
            # Establish the dirty-state precondition explicitly: an inactive
            # layer setter alone did not mark this native document edited.
            f.parent.updateChangeCount_(0)  # NSChangeDone, test fixture only

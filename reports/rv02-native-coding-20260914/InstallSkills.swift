...
@main struct RV02InstallSkills {
 static func main() throws {
  ...
  // Refuse unrelated replacements: the authorized legacy OpenType folder is
  // the only non-current, non-owned conflict this installation may replace.
  ...
  guard conflicts.isEmpty || replace && conflicts == ["glyphs-mcp-opentype-features"] else {
   throw NSError(domain:"RV02",code:1,userInfo:[NSLocalizedDescriptionKey:"Unreviewed skill conflicts: \(conflicts)"])
  }
  for legacy in InstallerPayload.legacyManagedSkillNames {
   guard !FileManager.default.fileExists(atPath: destination.appendingPathComponent(legacy).path) else {
    throw NSError(domain:"RV02",code:5,userInfo:[NSLocalizedDescriptionKey:"Unrelated legacy skill must be preserved: \(legacy)"])
   }
  }
  ...
  guard destination.path == FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".codex/skills").path else {
   throw NSError(domain:"RV02",code:2,userInfo:[NSLocalizedDescriptionKey:"Use the existing installer tests for temporary destinations"])
  }
  let result=try AgentSkillBundleInstaller(log: { print($0) }).installCodexSkills(payload:payload,overwriteExisting:replace)
  guard result.conflicts.isEmpty else { throw NSError(domain:"RV02",code:3,userInfo:[NSLocalizedDescriptionKey:result.summary]) }
  let rows = try payload.managedSkillDirectories().map { source -> [String:String] in
   ...
   guard expected == actual else { throw NSError(domain:"RV02",code:4) }

...
description: Connect Codex, ChatGPT, Claude, Cursor, or another MCP client to Glyphs MCP.
---
...
For the repository plugin, embedded feedback panel, and bundled skills, see
...
Cursor supports Streamable HTTP MCP servers through project or global
`mcp.json` files. Add a project connection at `.cursor/mcp.json`:
...
Use `~/.cursor/mcp.json` instead when the connection should be available across
projects. Reload Cursor, open its MCP settings or **Available Tools**, and
confirm that `glyphs-mcp-server` is enabled.

The Glyphs MCP installers do not currently write Cursor configuration. Preserve
other servers if you edit either file manually. See
[Cursor's MCP documentation](https://docs.cursor.com/context/model-context-protocol)
for current configuration locations and client behavior.

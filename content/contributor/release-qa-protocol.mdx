...
- `TOOL_CATALOG` contains 87 active entries: 76 `model+app` and 11
...
kerning, and LitSquare calls, inspect both MCP content channels.
...
Use one disposable multi-master glyph with corresponding closed paths, plus
open, ambiguous, stale, and incompatible fixtures. Record complete layer and
file signatures before each row.
...
- Install/upgrade both Glyphs targets with the release installer. Confirm
  ...
  and build number agree. Each host resolves version `1.11.1`, the same 11
  skills, and the same localhost MCP endpoint (the version script updates this
  sentence for the candidate).
- With the Glyphs 4 Quick Look and thumbnail Finder extensions active, confirm
  that neither installer target is marked running while both main applications
  are closed. Then run Glyphs 3 and Glyphs 4 separately and confirm that only
  the corresponding installer target is blocked and that quitting clears it.
...
[skill-routing prompt fixture](https://github.com/thierryc/Glyphs-mcp/blob/main/src/glyphs-mcp/tests/fixtures/llm_skill_routing.json)
as a fresh prompt without exposing its expected fields to the agent. Compare
the observed result with all of these fields before marking the case passed:

- `expected_skill`, including the explicit no-Glyphs-skill negative case
- `expected_action` and `expected_tools`
- `approval` and `snippet_only`
- the human-readable `expected_result`
- every entry in `forbidden_skills` and `forbidden_tools`

Record the host, client/model version, chosen skill, tool sequence, whether any
code executed, whether approval was requested, and the final pass/fail result.
A model judging its own response is useful iteration evidence but does not
replace a fresh-host or independent-model pass.

Then run the repository tool-routing fixture plus these broader real-client
prompts:

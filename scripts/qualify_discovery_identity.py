...
async def measure(endpoint, expected, metadata=None):
    ...
    for phase in ('first','warmup','trial-1','trial-2','trial-3','trial-4','trial-5'):
        ...
        async with Client(endpoint, timeout=10, init_timeout=10) as client:
            ...
            assert {t.name for t in tools} == {'get_status','list_documents','read_entities','start_job','get_job','apply_job','accept_job','discard_job','save_document','start_edit_workflow','get_edit_workflow','respond_edit_workflow'}

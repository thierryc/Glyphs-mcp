...
class McpToolRegistrationTextTests(unittest.TestCase):
    ...
    def test_status_window_has_starting_state_and_pulsing_blue_dot(self) -> None:
        ...
        self.assertIn("def _finish_server_starting(self):", text)
        self.assertIn("self._starting_server = False", text)
        self.assertIn("self._begin_server_start_poll()", text)
        self.assertIn("server_lifecycle_state(server, thread)", text)
        self.assertIn("should_emit_start_success(", text)
        self.assertIn("self._server.should_exit = True", text)
        ...
        self.assertIn('status_state = "error"', text)
        self.assertIn('status_value = tr("status.error")', text)
        self.assertIn('"status.error"', i18n_text)
        self.assertIn('"error.startup_failed"', i18n_text)
        self.assertIn('"error.unexpected_exit"', i18n_text)
        self.assertIn("activity_value.cell().setWraps_(True)", text)
        self.assertIn("field.setToolTip_(activity_text or None)", text)
    ...
    def test_server_startup_has_stop_handle(self) -> None:
        ...
        self.assertIn("target=self._run_server_thread", text)
        self.assertIn("server.run()", text)
        self.assertIn('getattr(server, "started", False)', text)
        self.assertIn("self.logError(text)", text)
        self.assertIn("Uvicorn server thread exited before readiness.", text)
        self.assertIn("Uvicorn server thread exited after readiness.", text)

...
class MCPBridgePlugin(GeneralPlugin):
    ...
    def StartStopServer_(self, sender):
        ...
        if hasattr(self, "_server_thread") and self._server_thread.is_alive():
            print(
                f"✓ Glyphs MCP Server is running on port {getattr(self, '_port', '?')}."
            )
            print(
                f"  • SSE endpoint: http://127.0.0.1:{getattr(self, '_port', '?')}/sse"
            )
        ...
        if port is None:
            print(
                f"✗ No free port between {self.default_port} and {self.default_port + self.max_port_attempts - 1}"
            )
            return
            ...
            try:
                ...
                if tools:
                    ...
                    for tool_name in sorted(tools.keys()):
                        ...
                        doc = (
                            getattr(tool, "__doc__", None) or "No description available"
                        )
                        # Extract first line of docstring for brief description
                        brief_desc = (
                            doc.split("\n")[0].strip() if doc else "No description"

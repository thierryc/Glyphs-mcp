...
Agent plugins are a separate, optional setup. Version 1.11.1 includes one shared
...
- Uses one ordered path plan: external or repository-selected Python prioritizes
  its user site with that target's `Scripts/site-packages` as fallback; Python
  physically embedded in a Glyphs app prioritizes the Glyphs folder.
- Stops when the package copy that can win has an incompatible Python ABI or
  Mac architecture, an unapproved origin, or an escaping symlink. It reports
  lower-priority duplicates without deleting or moving them.

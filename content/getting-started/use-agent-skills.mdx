...
encoding, kerning, spacing, master compatibility, outline work, an italic first pass, or Glyphs
...
Code, Cursor, and GitHub Copilot CLI. All four manifests use version `2.0.0`
...
server reports the selected host runtime: pinned 1.11 in Glyphs 3.5 and 2.0 in
Glyphs 4.
...
For a manual global Cursor install, copy the 18 skill directories from
...
| `glyphs-mcp-color-font` | Palette, SVG, bitmap color-layer, fallback, and color-export evidence review | `Use $glyphs-mcp-color-font to audit this color-font source without changing it.` |
...
| `glyphs-mcp-export-validation` | Source export readiness and existing-binary validation | `Use $glyphs-mcp-export-validation to validate this source or existing font binary without exporting implicitly.` |
| `glyphs-mcp-master-compatibility` | Diagnostic-first repair of glyph compatibility across masters | `Use $glyphs-mcp-master-compatibility to diagnose my selected glyph and safely make its masters compatible.` |
...
| `glyphs-mcp-scripting` | Staged document Python, reviewed open-world fallback, native safety repair, and verified rollback | `Use $glyphs-mcp-scripting because the typed tools do not fit this document edit.` |
| `glyphs-mcp-opentype-features` | Focused OpenType feature and stylistic-set inspection | `Use $glyphs-mcp-opentype-features to inspect the stylistic sets in this document.` |
| `glyphs-mcp-production-audit` | Cross-domain audit routing and evidence aggregation | `Use $glyphs-mcp-production-audit to coordinate a production review without changing the source.` |
| `glyphs-mcp-unicode-semantics` | Unicode mappings, controls, whitespace, marks, normalization, and fallback behavior | `Use $glyphs-mcp-unicode-semantics to audit this font's Unicode semantics without changing it.` |
| `glyphs-mcp-variable-font` | Axes, masters, instances, mappings, special layers, and variable readiness | `Use $glyphs-mcp-variable-font to audit this variable-font source without repairing it.` |
| `glyphs-mcp-maintainer-feedback` | Redacted reproducible reports for maintainers | `Use $glyphs-mcp-maintainer-feedback to prepare an issue-quality report.` |
...
| `glyphs-mcp-spacing` | Grid-aware width/foreground translation and atomic applies | `Use the glyphs-mcp-spacing skill to review exact-layer spacing and optional foreground translation before an approved atomic apply.` |
| `glyphs-mcp-release` | Version, docs, packaging, validation, signing, and publication gates | `Use $glyphs-mcp-release to prepare version 2.0.0 and stop before external publication.` |
...
- For Python document edits, review the staged semantic diff and confirm its
  exact stored request; use `live_open_world` only for effects staging cannot cover.
...
- Create reusable scripts and plug-ins in the workspace; install or run them only after a separate explicit request.
...
- [Glyphs MCP v2 command set](../reference/command-set-v2.mdx)
